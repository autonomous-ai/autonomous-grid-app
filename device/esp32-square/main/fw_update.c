...
rm device/esp32-square/sdkconfig && idf.py reconfigure"
#endif
...
// credit window (docs/protocol.md, "Firmware update"). Acking rarely means the app is allowed to have
...
// docs/protocol.md asks for ("the device verifies sha256 over what it actually wrote").
//
...
void fw_on_offer(const char *version, int size, const char *sha256)
{
    ...
    // audio the user has already spoken). Declining is simply NOT ANSWERING (docs/protocol.md §2): the
    // app offers again on the next hello, which is fifteen seconds away.
    //
    // ui_awaiting_answer() used to be a third condition here, for a question the agent had put on screen.
    // That screen went with the AskUserQuestion port — the protocol carries no message for it — so the
    // question it guarded against cannot arise.
    if (ui_any_project_busy() || voice_active()) {
        ESP_LOGI(TAG, "offer of %s declined for now — %s", version,
                 voice_active() ? "a voice turn is in flight" : "a turn is running");
        return;
    }
    ...
    // (docs/protocol.md §2), so doing the erase first means the first slice arrives at a partition that
