...
void ram_telemetry_periodic(const char *tag)
{
    ...
    // The long-lived tasks worth a high-water mark. Names must match xTaskCreate exactly — a typo here
    // costs nothing visible, it just silently stops reporting the one task you were worried about.
    //
    // The reference firmware measured 1,332 B free on a 6 KiB stack here and raised it to 7 KiB; that is
    // the kind of thing this line exists to catch, and it is only caught if the name is right.
