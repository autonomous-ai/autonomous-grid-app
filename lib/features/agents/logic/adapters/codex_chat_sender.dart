...
class CodexChatSender implements ChatSender {
  ...
  Stream<ChatSendUpdate> send({
    ...
    // The grid, the model's window and Grid's own tools — the same preparation
    // the terminal lane runs, so a chat and a terminal answer on the same grid
    // with the same tools. Started here rather than at launch so a run that
    // never reaches an agent never opens a socket.
