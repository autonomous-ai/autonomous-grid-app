...
final codexAgentServiceProvider = Provider<CodexAgentService?>((ref) {
  final path = ref.watch(agentToolPathProvider(AgentTool.codex));
  return path == null ? null : CodexAgentServiceImpl(path);
});

/// The read-only working root the agent runs in, created on first read. Shared
/// by both agent backends (they run one at a time).
...
/// upserts each step as codex reports it. (Hermes's one-shot has no event
/// stream, so it leaves this empty.)
