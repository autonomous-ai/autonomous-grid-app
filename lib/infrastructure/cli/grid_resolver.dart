...
import 'grid_resolution.dart';
import 'grid_system_lookup.dart';
...
class GridResolver {
  GridResolver({this.configuredPath, String? Function()? pathLookup})
    : _pathLookup = pathLookup ?? resolveGridFromSystem;
  ...
  String? resolve() => resolveDetailed().path;

  /// [resolve] with the reason attached: which candidates were looked at, and
  /// whether the one we shipped was rejected for the CPU it was built for.
  GridResolution resolveDetailed() {
    final probed = <String>[];
    ...
    for (final candidate in _pinnedCandidates()) {
      ...
      _note(
        probed,
        candidate,
        path == null ? 'not an executable file' : 'used',
      );
      if (path != null) return GridResolution(path: path, probed: probed);
    }
    ...
    for (final candidate in _bundledCandidates()) {
      final path = _usablePath(candidate);
      if (path == null) {
        _note(probed, candidate, 'not in this app bundle');
        continue;
      }
      if (!_runnableHere(path)) {
        wrongArch ??= path;
        _note(probed, candidate, "built for a CPU this Mac can't run");
        continue;
      }
      _note(probed, candidate, 'used');
      return GridResolution(path: path, probed: probed);
    }

    final system = _pathLookup();
    _note(
      probed,
      system ?? 'well-known install dirs + login-shell PATH',
      system == null ? 'no grid installed' : 'used',
    );
    return GridResolution(
      path: system,
      probed: probed,
      // Only worth reporting when it was the last chance: a system `grid` that
      // answered makes the unrunnable sidecar a non-event.
      wrongArchSidecar: system == null ? wrongArch : null,
    );
  }

  /// Records one candidate and what became of it. Null/empty candidates (an
  /// unset `GRID_BIN`) are not places we looked, so they leave no trace.
  static void _note(List<String> probed, String? candidate, String outcome) {
    if (candidate == null || candidate.isEmpty) return;
    probed.add('$candidate — $outcome');
  }
  ...
  String? _usablePath(String? candidate) {
    ...
    if (!isExecutableFile(file) || _isSelf(file)) return null;
