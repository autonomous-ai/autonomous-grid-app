...
import '../widgets/extension_tile_surface.dart';
import '../widgets/panel_visibility.dart';
import 'panel_tab_strip.dart';
import 'panel_tabs.dart';

/// How a [PanelTab] becomes a widget.
///
/// Supplied by whoever mounts the panel rather than resolved in here: the tabs,
/// the strip and the launcher are the same everywhere, but *what* a Review or a
/// Terminal is rooted at is not — the chat's panel opens onto the folder the
/// assistant works in, a project's onto that project's copy on this computer.
typedef PanelTabViewBuilder =
    Widget Function(PanelTab tab, {required VoidCallback onClose});

/// The work surface beside a conversation: where a review, a terminal or a file
/// listing opens, so looking at one doesn't push the conversation off the
/// screen.
///
/// Two states, and the tab strip decides which:
///
///  - **No tab open** — the launcher, the list of what this panel can open.
///  - **A tab selected** — that feature's own surface.
///
/// Deliberately without panel controls of its own: the buttons that move the
/// panels live with the screen that owns them. A panel carrying its own copy
/// puts a second set directly under the first, and the user has to work out
/// whether the two rows mean different things. A *surface* inside a tab may
/// carry its own header — that belongs to the surface, not to the panel.
class PanelSurface extends ConsumerWidget {
  const PanelSurface({
    super.key,
    required this.host,
  ...
  /// Which panel this is drawing.
  final PanelHost host;

  /// What each tab is rooted at — see [PanelTabViewBuilder].
  final PanelTabViewBuilder viewBuilder;
  ...
  Widget build(BuildContext context, WidgetRef ref) {
    ...
    final open = ref.watch(panelOpenProvider(host));
    ...
    // into the conversation would go to a terminal nobody can see.
    return ExcludeFocus(
      excluding: !open,
      child: Column(
        ...
        children: [
          ...
          if (state.tabs.isNotEmpty) ...[
            PanelTabStrip(host: host, onRaisedSurface: onRaisedSurface),
            const Divider(height: 1),
          ],
          Expanded(
            child: active == null
                ? _Launcher(host: host, onRaisedSurface: onRaisedSurface)
                : _OpenTabs(
                    host: host,
                    state: state,
                    active: active,
      ...
      for (final tab in state.tabs)
        ...
        KeyedSubtree(
          ...
          child: _Tab(
            ...
            tab: tab,
            onClose: () =>
                ref.read(panelTabsProvider(host).notifier).close(tab.id),
            viewBuilder: viewBuilder,
          ),
        ),
    ],
...
class _Tab extends StatelessWidget {
  ...
  @override
  Widget build(BuildContext context) => PanelTabVisible(
    visible: visible,
    child: TickerMode(
      enabled: visible,
      child: ExcludeFocus(
        excluding: !visible,
        child: viewBuilder(tab, onClose: onClose),
      ),
    ),
  );
}
...
class _Launcher extends ConsumerWidget {
  const _Launcher({required this.host, required this.onRaisedSurface});
  ...
  @override
  Widget build(BuildContext context, WidgetRef ref) {
    final open = ref.read(panelTabsProvider(host).notifier).open;
    // Centred when there's room, scrolled when there isn't: the rows need
    // ~260px, and a short window with another panel open leaves less than
