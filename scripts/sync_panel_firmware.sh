...
# carrying and offers to fix a mismatch over the same cable
# (docs/panel-protocol.md §2, "Firmware update").
#
# Why a copy rather than an asset line pointing straight at the build tree:
# `device/esp32-circle/build/` is gitignored — an ESP-IDF build tree is tens of
...
# flashed a panel, which is most people working on this app. assets/panel/README.md
# says the same thing from the other side, which is why that directory has to keep
# existing even when it holds no image.
#
...
SRC="device/esp32-circle/build/grid_panel.bin"
DST="assets/panel/grid_panel.bin"

if [ ! -f "$SRC" ]; then
  echo "no $SRC — build it first: cd device/esp32-circle && idf.py build" >&2
  exit 1
fi
...
# (docs/panel-protocol.md, "Firmware update"). Sync, rebuild, THEN flash the panel.
