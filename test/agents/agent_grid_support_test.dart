...
void main() {
  group('which agents a grid can run', () {
    test('Codex needs a grid that answers on the Responses API', () {
      ...
      expect(agentNeedsResponses(AgentTool.codex), isTrue);
      expect(
        agentRunsOnGrid(AgentTool.codex, advertisesResponses: false),
        isFalse,
      );
      expect(
        agentRunsOnGrid(AgentTool.codex, advertisesResponses: true),
        isTrue,
      );
    });
    ...
    test('Hermes runs on any grid — it speaks both dialects', () {
      ...
      expect(agentNeedsResponses(AgentTool.hermes), isFalse);
      for (final flag in <bool?>[true, false, null]) {
        expect(
          agentRunsOnGrid(AgentTool.hermes, advertisesResponses: flag),
          isTrue,
        );
      }
    });
    ...
    test('a relay that never reported the flag lets Codex try anyway', () {
      ...
      expect(
        agentRunsOnGrid(AgentTool.codex, advertisesResponses: null),
        isTrue,
      );
    });
