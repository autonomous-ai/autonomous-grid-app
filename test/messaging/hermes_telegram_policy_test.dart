...
import 'package:grid_app/infrastructure/cli/hermes_telegram_policy.dart';
...
void main() {
  ...
  setUp(() async {
    tmp = await Directory.systemTemp.createTemp('grid_telegram_policy_test');
    config = File('${tmp.path}/.hermes/config.yaml');
  });
  tearDown(() => tmp.delete(recursive: true));

  HermesTelegramPolicy policy() => HermesTelegramPolicy(home: tmp.path);
  ...
  test('restrict takes the command tools away from a remote Telegram message, '
      'rather than trusting a bot on the internet not to use them', () async {
    await policy().restrict();

    final telegram = at(['platform_toolsets', 'telegram']) as List;
    expect(telegram, isNot(contains('terminal')));
    expect(telegram, isNot(contains('code_execution')));
    expect(telegram, isNot(contains('browser')));
    expect(telegram, isNot(contains('computer_use')));
  ...
  test('a bot connected before the limit existed reads as unrestricted, and '
    ...
    expect(await policy().isRestricted, isFalse);

    await policy().restrict();
    expect(await policy().isRestricted, isTrue);
  });

  test("Hermes's default (a config that never mentions Telegram) is not "
...
''');
  ...
  test('restricting is idempotent — a second call keeps a valid, terminal-free '
    ...
    expect(telegram, contains('file'));
    expect(telegram, isNot(contains('terminal')));
    expect(await p.isRestricted, isTrue);
  });

  test('the Telegram limit and the scheduler limit share one config without '
