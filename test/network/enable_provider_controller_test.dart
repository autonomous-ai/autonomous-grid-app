...
const _addArgs = ['members', 'add', _net, _email, '--role', 'provider'];
...
void main() {
  test('grants the provider role, syncs, then reports done', () async {
    ...
    container.listen(
      enableProviderControllerProvider,
  ...
  test('fails fast when grid is absent', () async {
    ...
    expect(
      container.read(enableProviderControllerProvider),
      isA<EnableFailed>(),
    );
  });
}
